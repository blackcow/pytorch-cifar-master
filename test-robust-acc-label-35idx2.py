...
#         '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_el_li3_CIFAR10/seed1/model-wideres-epoch100.pt',
#        '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_label_smooth35_CIFAR10/seed1/model-wideres-epoch100.pt',
#        '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_CIFAR10/seed1/model-wideres-epoch100.pt'
#        ]

ckptlist =['../Fair-AT/model-cifar-wideResNet/preactresnet/ST_el_li3_CIFAR10/seed1/model-wideres-epoch100.pt',
        '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_el_li3_CIFAR10/seed2/model-wideres-epoch100.pt',
       '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_el_li3_CIFAR10/seed3/model-wideres-epoch100.pt',
       '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_el_li3_CIFAR10/seed4/model-wideres-epoch100.pt',
           '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_el_li3_CIFAR10/seed5/model-wideres-epoch100.pt'
       ]
...
#             '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_CIFAR10/seed4/model-wideres-epoch100.pt',
#             '../Fair-AT/model-cifar-wideResNet/preactresnet/ST_CIFAR10/seed5/model-wideres-epoch100.pt']
...
def loadmodel_preactresnte(i, ckptlist, factor):
...
def test(writer, net, model_name, epsilon, AT_method):
    ...
    with torch.no_grad():
        ...
        input_tmp = torch.cat(input[:-1])
        input = torch.cat((input_tmp, input[-1]), dim=0)
        ...
        for i in [3, 5]:
            ...
            img_35.append(torch.index_select(input, 0, idx).cpu().numpy())  # 3,5 img data
            # out_35
        ...
        print()
    return wrong_idx3, wrong_idx5, img_35


# 统计出现次数
def count(wrong_idx_all):
    y = wrong_idx_all[0]
    for i in range(1, len(wrong_idx_all)):
        y = np.concatenate([y, wrong_idx_all[i]], axis=0)

    # y1 = np.concatenate([wrong_idx_all[0], wrong_idx_all[1], wrong_idx_all[2], wrong_idx_all[3], wrong_idx_all[4]], axis=0)
    y = sorted(y)
    # print(y)
    # 统计出现的元素有哪些
...
def main():
    ...
    if args.factors == 'model':
        for i in range(model_num):
            ...
            net = loadmodel_preactresnte(i, ckptlist, factor)
            # test robust fair model
            # net = loadmodel_robustfair(i, factor)
            wrong_idx3, wrong_idx5, img_35 = test(writer, net, 'model_name', factor[0], args.AT_method)
    ...
    # wrong_intersection_idx3 = list(set(wrong_idx3_all[0]).intersection(set(wrong_idx3_all[1])))
    # wrong_intersection_idx5 = list(set(wrong_idx5_all[0]).intersection(set(wrong_idx5_all[1])))
    # 绘制被多个 model 错误分类的图像
    # 错误图像的 idx，在 label3、5 data 下
    # wrong_intersection_idx3 = list(set(wrong_idx3_all[0]).intersection(wrong_idx3_all[1], wrong_idx3_all[2]))
    # wrong_intersection_idx5 = list(set(wrong_idx5_all[0]).intersection(wrong_idx5_all[1], wrong_idx5_all[2]))
    # 比较 5 个模型
    wrong_intersection_idx3 = list(set(wrong_idx3_all[0]).intersection(wrong_idx3_all[1], wrong_idx3_all[2],wrong_idx3_all[3],wrong_idx3_all[4]))
    wrong_intersection_idx5 = list(set(wrong_idx5_all[0]).intersection(wrong_idx5_all[1], wrong_idx5_all[2],wrong_idx5_all[3],wrong_idx5_all[4]))

    # wrong_intersection_idx3 = list(
    #     set(wrong_idx3_all[0]).intersection(wrong_idx3_all[1], wrong_idx3_all[2], wrong_idx3_all[3]))
    # wrong_intersection_idx5 = list(
    #     set(wrong_idx5_all[0]).intersection(wrong_idx5_all[1], wrong_idx5_all[2], wrong_idx5_all[3]))
    wrong_intersection_idx3 = sorted(wrong_intersection_idx3)
    wrong_intersection_idx5 = sorted(wrong_intersection_idx5)
    img3 = img_35[0][wrong_intersection_idx3]
    img5 = img_35[1][wrong_intersection_idx5]
    ...
    plot_images_labels_prediction(img3, num=len(img3))
    plot_images_labels_prediction(img5, num=len(img5))
    print('For 3 label:')
...
def plot_images_labels_prediction(images, num):
    fig = plt.gcf()
    fig.set_size_inches(10, 10)
    # if num > 25: num = 25
    for i in range(num):
        ax = plt.subplot(math.sqrt(num)+1, math.sqrt(num)+1, i + 1)
        # img = ndimage.rotate(images[idx], -90)
        pic = images[i].transpose(1, 2, 0)
