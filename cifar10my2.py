...
class CIFAR10MY(data.Dataset):
    ...
    def __init__(self, root, args, train=True,
        ...
        # 确定删除 del label 的全部元素的 index
        label_del_idx = self.args.dele
        start_dele = label_del_idx * 5000
        end_dele = int(start_dele + 5000)

        # 删除元素，label 和 data
        rm_imlabel = np.arange(start, end, 1)
        rm_dele = np.arange(start_dele, end_dele, 1)
        rm = np.append(rm_imlabel, rm_dele)

        # self.targets1 = np.delete(self.targets, rm)
        self.data = np.delete(self.data, rm, axis=0)
        # targets = self.targets.pop(rm)
