...
parser.add_argument('--imlabel', default=6, type=int, help='Label of the deleted training data')
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt_rmlabel'+str(args.imlabel)+'_percent'+str(args.percent)+'.pth') # 会覆盖掉前面的保存
        best_acc = acc
        best_epoch = epoch
